  ...
  {moment(message.timestamp).format("LT")}
    ...
    useEffect(()=>{
      ...
      const getChannelMessages=async()=>{
        try{
          const response=await axios.get(`${serverUrl}/api/channel/get-channel-messages/${selectedChatData._id}`,{withCredentials:true})
          if(response.data.messages){
            setSelectedChatMessages(response.data.messages)
...
if(scrollRef.current){
  ...
  const renderChannelMessages=(message:any)=>{
     ...
     return <div className={` mt-5  ${message.sender._id!==userInfo?._id?"text-left":"text-right"}`}>
      ...
      <div className={`${message.sender._id===userInfo?._id?"bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50 ":"bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20"} border inline-block p-4 rounded my-1 max-w-[50%] break-words ml-9`}>
      {message.content}
    </div>
}
{
  message.messageType==="file" && <div className={`${message.sender._id===userInfo?._id?"bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50 ":"bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20"} border inline-block p-4 rounded my-1 max-w-[50%] break-words`}>
     {
      checkIfImage(message.fileUrl)?<div className="cursor-pointer" onClick={()=>{setShowImage(true);setImageUrl(message.fileUrl)}}>
...
{message.sender._id!==userInfo?._id? <div className='flex items-center justify-start gap-3'>
  <Avatar className="h-8 w-8  rounded-full overflow-hidden">
                        {message.sender.image && (
                          ...
                          <AvatarFallback
                            className={` uppercase h-8 w-8 md:w-12 text-lg  flex items-center justify-center rounded-full ${getColor(
                              message.sender.color? message.sender.color: 0
                            )}`}
                          >
